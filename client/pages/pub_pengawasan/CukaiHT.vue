      ...
      <table class="table table-bordered">
        ...
        <tbody>
          <tr v-for="(v, k) in data.data" :key="k">
            ...
            <td>{{ v.name }}</td>
            <td>{{ v.sbp }} Dokumen</td>
            <td class="text-right">{{ v.total | formatNumber}} Liter</td>
            <td>{{ v.follow_up }} Dokumen</td>
            <td class="text-right">{{ v.item_value | formatNumber}} Rupiah</td>
            <td class="text-right">{{ v.loss_value | formatNumber}} Rupiah</td>
          </tr>
        </tbody>
      </table>
    </div>
...
export default {
  ...
  head () {
    return { title: 'Pengawasan Cukai HT' }
  },
  ...
  methods: {
    async getList (page) {
      const response = await axios.get(`/pub/control?type=cht&size=10&page=${page}`);
