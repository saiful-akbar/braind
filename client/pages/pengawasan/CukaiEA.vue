<template>
  <card title="Data Pengawasan Cukai EA">
    <form @submit.prevent="action" @keydown="form.onKeydown($event)">
      ...
      <div class="form-group row">
        <div class="col-md-9 ml-md-auto">
          ...
          <export-button url="/control/export/excel?type=cea" />
        </div>
      </div>
    </form>
    <import-buttons importData="controls-ea" @update="getList(1)" />
  ...
  methods: {
    ...
    async getList(page) {
      ...
      const response = await axios.get(
        `/control?type=cea&size=10&page=${page}&${q}`
      );
