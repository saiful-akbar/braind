<template>
  <card title="Data Pengawasan Export">
    <form @submit.prevent="action" @keydown="form.onKeydown($event)">
      ...
      <div class="form-group row">
        <div class="col-md-9 ml-md-auto">
          ...
          <export-button url="/control/export/excel?type=exp" />
        </div>
      </div>
    </form>
    <import-buttons importData="controls-export" @update="getList(1)" />
...
export default {
  ...
  head() {
    return { title: "Pengawasan Export" };
  },
  ...
  methods: {
    ...
    async getList(page) {
      ...
      const response = await axios.get(
        `/control?type=exp&size=10&page=${page}&${q}`
      );
