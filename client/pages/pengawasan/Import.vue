    ...
    <form @submit.prevent="action" @keydown="form.onKeydown($event)">
      ...
      <div class="form-group row">
        <div class="col-md-9 ml-md-auto">
          ...
          <export-button url="/control/export/excel?type=imp" />
        </div>
      </div>
    </form>
    <import-buttons importData="controls-import" @update="getList(1)" />
      ...
      <table class="table table-bordered">
        ...
        <tbody>
          <tr v-for="(v, k) in data.data" :key="k">
            ...
            <td>{{ v.follow_up }}</td>
            <td class="text-right">{{ format(v.item_value) }}</td>
            <td>
  ...
  methods: {
    ...
    async getList(page) {
      ...
      const response = await axios.get(
        `/control?type=imp&size=10&page=${page}&${q}`
      );
