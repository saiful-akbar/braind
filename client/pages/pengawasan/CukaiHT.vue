<template>
  <card title="Data Pengawasan Cukai HT">
    <form @submit.prevent="action" @keydown="form.onKeydown($event)">
      ...
      <div class="form-group row">
        <div class="col-md-9 ml-md-auto">
          ...
          <export-button url="/control/export/excel?type=cht" />
        </div>
      </div>
    </form>
    <import-buttons importData="controls-ht" @update="getList(1)" />
  ...
  methods: {
    ...
    async getList(page) {
      ...
      const response = await axios.get(
        `/control?type=cht&size=10&page=${page}&${q}`
      );
