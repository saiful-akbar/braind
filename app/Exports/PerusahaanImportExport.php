...
class PerusahaanImportExport implements FromView, WithStyles, ShouldAutoSize
{
    use Exportable;
    ...
    public function __construct(private Request $request, private MenuUser $access)
    {
        ...
        // Ambil data pada tabel perusahaan_import dan join dengan tabel kantor.
        // lalu buat filter berdasarkan periode waktu.
        $this->query = PerusahaanImport::select($columns)
            ->leftJoin('kantor', 'perusahaan_import.kantor_id', '=', 'kantor.id')
        ...
        if (!empty($request->query('search'))) {
            $this->query->where(function (Builder $query) use ($request): void {
                ...
                $query->where('perusahaan_import.nama_perusahaan', 'like', "%{$search}%")
    ...
    function view(): View
    {
        return view('exports.perusahaan-import-export', [
            'data' => $this->query->get(),
        ]);
    }
}
