...
class Dashboard implements Reader
{
    ...
    public function __construct($divId, Request $request)
    {
        ...
        $this->data = ImportCompany::whereNull('import_companies.deleted_at')
            ->select('name', DB::raw("$ord as value"))
            ->orderBy("import_companies.$ord", 'desc');
        if ($divId && $divId > 0) {
            $this->data->where('import_companies.division_id', $divId);
        }
        $user = auth()->user();
        if ($user && $user->division_id) {
            $this->data->where('import_companies.division_id', $user->division_id);
        }
    }
