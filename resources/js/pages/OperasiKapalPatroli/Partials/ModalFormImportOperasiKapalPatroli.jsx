...
import { closeFormlImport } from "@/redux/reducers/operasiKapalPatroliReducer";
...
/**
 * Komponen modal form untuk import excel data sarana oprasi kapal patroli.
 */
const ModalFormImportOperasiKapalPatroli = memo(() => {
  const { app } = usePage().props;
  const { params } = app.url;
  const dispatch = useDispatch();
  const { open } = useSelector((state) => state.operasiKapalPatroli.import);
  ...
  const handleDownloadTemplate = useCallback(async () => {
    ...
    try {
      const response = await axios({
        method: "get",
        url: route("operasi-kapal-patroli.import.template"),
        responseType: "blob",
      });
      ...
      if (response.status === 200) {
        ...
        saveAs(response.data, "template_import_operasi_kapal_patroli.xlsx");
  ...
  const handleSubmit = (e) => {
    ...
    const url = route("operasi-kapal-patroli.import", {
      _query: params,
    });
...
export default ModalFormImportOperasiKapalPatroli;
