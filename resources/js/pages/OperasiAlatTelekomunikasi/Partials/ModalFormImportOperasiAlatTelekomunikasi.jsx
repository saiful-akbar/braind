...
/**
 * Komponen modal form untuk import excel data sarana oprasi alat telekomunikasi.
 */
const ModalFormImportOperasiAlatTelekomunikasi = memo(() => {
  const { app } = usePage().props;
  const { params } = app.url;
  const dispatch = useDispatch();
  const { open } = useSelector(
    (state) => state.operasiAlatTelekomunikasi.import
  );
  ...
  const handleDownloadTemplate = useCallback(async () => {
    ...
    try {
      const response = await axios({
        method: "get",
        url: route("operasi-alat-telekomunikasi.import.template"),
        responseType: "blob",
      });
      ...
      if (response.status === 200) {
        ...
        saveAs(
          response.data,
          "template_import_operasi_alat_telekomunikasi.xlsx"
        );
  ...
  const handleSubmit = (e) => {
    ...
    const url = route("operasi-alat-telekomunikasi.import", {
      _query: params,
    });
...
export default ModalFormImportOperasiAlatTelekomunikasi;
