...
import { closeFormlImport } from "@/redux/reducers/perusahaanExportReducer";
 ...
 */
const ModalFormImportPerusahaanExport = memo(() => {
  const { app } = usePage().props;
  const { params } = app.url;
  const { open } = useSelector((state) => state.perusahaanExport.import);
    ...
    if (!form.processing) {
      try {
        const response = await axios({
          method: "get",
          url: route("perusahaan-export.import.template"),
          responseType: "blob",
        });

        if (response.status === 200) {
          saveAs(response.data, "template_import_perusahaan_export.xlsx");
  ...
  const handleSubmit = (e) => {
    ...
    const url = route("perusahaan-export.import", {
      _query: params,
    });
...
export default ModalFormImportPerusahaanExport;
