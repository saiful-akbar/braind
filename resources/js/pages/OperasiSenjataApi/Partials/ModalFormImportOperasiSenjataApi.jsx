...
import { closeFormlImport } from "@/redux/reducers/operasiSenjataApiReducer";
...
/**
 * Komponen modal form untuk import excel data sarana oprasi senjata api.
 */
const ModalFormImportOperasiSenjataApi = memo(() => {
  const { app } = usePage().props;
  const { params } = app.url;
  const dispatch = useDispatch();
  const { open } = useSelector((state) => state.operasiSenjataApi.import);
  ...
  const handleDownloadTemplate = useCallback(async () => {
    ...
    try {
      const response = await axios({
        method: "get",
        url: route("operasi-senjata-api.import.template"),
        responseType: "blob",
      });
      ...
      if (response.status === 200) {
        ...
        saveAs(response.data, "template_import_operasi_senjata_api.xlsx");
  ...
  const handleSubmit = (e) => {
    ...
    const url = route("operasi-senjata-api.import", {
      _query: params,
    });
...
export default ModalFormImportOperasiSenjataApi;
